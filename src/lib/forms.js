 ...
 * beneran, bukan cuma disembunyiin di tampilan.
 *
 * Balikin satu entri PER RONDE (lesson) -- satu ronde bisa punya banyak
 * pengirim -- lengkap dengan `form` mentah (buat FormSummary) dan
 * `responses` mentah (buat dihitung agregatnya di sana, bukan di sini):
 *   { lessonId, title, formId, form, responses: [{ answers, created_at }] }
 */
export async function getMyFeedback(courseId) {
  ...
  for (const r of rows) {
    ...
    byLesson
      .get(r.lesson_id)
      .responses.push({ answers: r.answers, created_at: r.created_at });
  }
  const rounds = [...byLesson.values()];

  const formIds = [...new Set(rounds.map((r) => r.formId).filter(Boolean))];
  const forms = await Promise.all(
    formIds.map((id) => getForm(id).catch(() => null))
  );
  const formsById = new Map(forms.filter(Boolean).map((f) => [f.id, f]));

  return rounds.map((r) => ({ ...r, form: formsById.get(r.formId) ?? null }));
}

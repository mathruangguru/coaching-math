    ...
    .select(
      `id, title, description, icon, announcement,
       sections:coaching_course_sections (
         ...
         items:coaching_lessons ( id, type, title, duration, url, question_set_id, form_id, prompt, content, publish_status, access_open, access_opens_at, access_closes_at, target_user_id, target_name, allow_download, position )
       )`
    )
    .eq("id", id)
    .maybeSingle();
